...
from fixlib.baseplugin import BasePlugin, BaseCollectorPlugin
from fixlib.baseresources import BaseCloud, BaseAccount, BaseRegion, BaseResource
from fixlib.config import Config
from fixlib.event import (
    Event,
    EventType,
    add_event_listener,
...
class SlackBotPlugin(BasePlugin):
    def __init__(self):
        ...
        add_event_listener(EventType.CLEANUP_FINISH, self.process_fix_events, blocking=False)

    def __del__(self):
        remove_event_listener(EventType.CLEANUP_FINISH, self.process_fix_events)
    ...
    def process_fix_events(self, event: Event):
        graph = event.data
        log.info("Checking for outstanding Slack notifications")
        self.update_users_groups_channels(graph)

        for node in graph.nodes:
            if isinstance(node, BaseResource) and len(node.event_log) > 0 and "fix:owner" in node.tags:
                cloud = node.cloud(graph)
                account = node.account(graph)
                region = node.region(graph)
                owner_tag = str(node.tags["fix:owner"])
                ...
                if not isinstance(destination, SlackUser):
                    log.error(f"Unable to determine Slack destination based on fix:owner tag value {owner_tag}")
                    continue
                ...
                slack_message = (
                    ...
                    f" was modified during the current fix run. Here is the "
                    f"event log:\n```\n{event_log_text}```"
                )
