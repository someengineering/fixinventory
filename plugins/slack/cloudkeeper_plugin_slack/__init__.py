...
class SlackCollectorPlugin(BaseCollectorPlugin):
    ...
    def list_conversations(self) -> List:
        ...
        response = self.client.conversations_list(exclude_archived='true', types='public_channel,private_channel',
                                                  limit=100)
        conversations = response.data.get('channels', [])
        while response.data.get('response_metadata', {}).get('next_cursor', '') != '':
            time.sleep(2)
            response = self.client.conversations_list(cursor=response.data['response_metadata']['next_cursor'],
...
class SlackBotPlugin(BasePlugin):
    ...
    @staticmethod
    def add_args(arg_parser: ArgumentParser) -> None:
        arg_parser.add_argument('--slack-bot-token', help='Slack Bot Token (default env $SLACK_BOT_TOKEN)',
                                default=os.environ.get('SLACK_BOT_TOKEN'), dest='slack_bot_token', type=str)
