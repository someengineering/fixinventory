...
class ExamplePersistentPlugin(BasePlugin):
    def __init__(self):
        ...
        add_event_listener(
            EventType.PROCESS_FINISH, self.example_event_handler, blocking=False
        )
    ...
    def example_event_handler(event: Event):
        if ArgumentParser.args.example_arg:
            ...
            for node in graph.search("kind", "example_account"):
                log.debug(
                    f"Found node {node.dname} of kind {node.kind} created {node.ctime}"
                )

    @staticmethod
    def add_args(arg_parser: ArgumentParser) -> None:
        arg_parser.add_argument(
            "--example-arg", help="Example Arg", default=None, dest="example_arg"
        )
