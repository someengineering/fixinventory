...
class TagValidatorPlugin(BaseActionPlugin):
    ...
    def invalid_expiration(self, graph: Graph, node: BaseResource, default_expiration: str) -> Optional[str]:
        """
        ...
        max_expiration_str: str = (
            value_in_path(self.config, ["accounts", cloud.id, account.id, "expiration"]) or default_expiration
        )
    ...
    def do_action(self, data: Json) -> None:
        ...
        default_expiration: str = value_in_path(self.config, ["default", "expiration"]) or "24h"
        graph = cg.graph(query)
        commands = []
        for node in graph.nodes:
            if node.protected or node._resotocore_query_tag != query_tag:
                continue
            if max_expiration := self.invalid_expiration(graph, node, default_expiration):
