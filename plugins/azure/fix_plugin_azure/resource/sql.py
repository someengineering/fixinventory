...
class AzureManagedInstancePecProperty:
    ...
    mapping: ClassVar[Dict[str, Bender]] =  {
        "id": S("id"),
        "private_endpoint": S("properties","privateEndpoint","id"),
        "private_link_service_connection_state": S("properties","privateLinkServiceConnectionState") >> Bend(AzureManagedInstancePrivateLinkServiceConnectionStateProperty.mapping),
        "provisioning_state": S("properties","provisioningState")
    }
    id: Optional[str] = field(default=None, metadata={'description': 'Resource ID.'})
    private_endpoint: Optional[str] = field(default=None, metadata={'description': ''})
...
class AzureSqlManagedInstance(MicrosoftResource):
    ...
    timezone_id: Optional[str] = field(default=None, metadata={'description': 'Id of the timezone. Allowed values are timezones supported by Windows. Windows keeps details on supported timezones, including the id, in registry under KEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Time Zones. You can get those registry values via SQL Server by querying SELECT name AS timezone_id FROM sys.time_zone_info. List of Ids can also be obtained by executing [System.TimeZoneInfo]::GetSystemTimeZones() in PowerShell. An example of valid timezone id is Pacific Standard Time or W. Europe Standard Time .'})  # fmt: skip
...
class AzureRecommendedAction:
    ...
    mapping: ClassVar[Dict[str, Bender]] = {
        ...
        "estimated_impact": S("properties","estimatedImpact") >> ForallBend(AzureRecommendedActionImpactRecord.mapping),
        ...
        "implementation_details": S("properties","implementationDetails") >> Bend(AzureRecommendedActionImplementationInfo.mapping),
    ...
    recommended_action_details: Optional[Dict[str, str]] = field(default=None, metadata={'description': 'Gets additional details specific to this recommended action.'})  # fmt: skip
    ...
    linked_objects: Optional[List[str]] = field(default=None, metadata={'description': 'Gets the linked objects, if any.'})
