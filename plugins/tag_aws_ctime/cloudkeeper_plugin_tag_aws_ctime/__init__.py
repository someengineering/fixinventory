...
from cloudkeeper_plugin_aws.resources import AWSALBTargetGroup, AWSEC2KeyPair
...
class TagAWSCtimePlugin(BasePlugin):
    def __init__(self):
        ...
        if ArgumentParser.args.tag_aws_ctime:
            ...
            add_event_listener(EventType.COLLECT_FINISH, self.aws_ctime_tagger, blocking=False, timeout=30)
        else:
            self.exit.set()
    ...
    def tag_ctime(self, graph: Graph):
        ...
        with graph.lock.read_access:
            for node in graph.nodes:
                if (
                    not isinstance(node, AWSALBTargetGroup)
                    and not isinstance(node, AWSEC2KeyPair)
                ):
                    continue
    ...
    def add_args(arg_parser: ArgumentParser) -> None:
        arg_parser.add_argument('--tag-aws-ctime', help='Tag AWS ctime (default: False)', dest='tag_aws_ctime', action='store_true', default=False)
