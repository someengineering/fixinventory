...
class AwsLambdaFunction(AwsResource, BaseServerlessFunction):
    ...
    metadata: ClassVar[Dict[str, Any]] = {"provider_link_tpl": "https://{region_id}.console.aws.amazon.com/lambda/home?region={region}#/functions/{id}", "arn_tpl": "arn:{partition}:lambda:{region}:{account}:function/{name}"}  # fmt: skip
    kind_description: ClassVar[str] = (
