...
class AwsSSMInstance(AwsResource):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ssm:{region}:{account}:instance/{name}"}  # fmt: skip
...
class AwsSSMDocument(AwsResource):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ssm:{region}:{account}:document/{name}"}  # fmt: skip
    mapping: ClassVar[Dict[str, Bender]] = {
        "id": S("Name"),
        "tags": S("Tags", default=[]) >> ToDict(),
        "name": S("Name"),
