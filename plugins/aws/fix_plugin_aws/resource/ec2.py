        ...
        queries.extend(
            [
                ...
                for name, metric_name in [
                    ("VolumeWriteBytes", MetricName.VolumeWrite),
                    ("VolumeReadBytes", MetricName.VolumeRead),
                ]
            ]
        )
        queries.extend(
            [
                ...
                for name, metric_name in [
                    ("VolumeWriteOps", MetricName.VolumeWrite),
                    ("VolumeReadOps", MetricName.VolumeRead),
                ]
            ]
        )
        queries.extend(
            [
                ...
                for name, metric_name in [
                    ("VolumeTotalWriteTime", MetricName.VolumeTotalWriteTime),
                    ("VolumeIdleTime", MetricName.VolumeIdleTime),
                ]
            ]
        )
        queries.extend(
...
class AwsEc2Snapshot(EC2Taggable, AwsResource, BaseSnapshot):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:snapshot/{id}"}  # fmt: skip
    kind_description: ClassVar[str] = (
...
class AwsEc2KeyPair(EC2Taggable, AwsResource, BaseKeyPair):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:keypair/{name}"}  # fmt: skip
    ...
    def collect_usage_metrics(self, builder: GraphBuilder) -> List[AwsCloudwatchQuery]:
        ...
        queries.extend(
            [
                ...
                for name, metric_name in [
                    ("NetworkPacketsIn", MetricName.NetworkIn),
                    ("NetworkPacketsOut", MetricName.NetworkOut),
                ]
            ]
        )
        ...
        queries.extend(
            [
                ...
                for name, metric_name in [
                    ("DiskReadOps", MetricName.DiskRead),
                    ("DiskWriteOps", MetricName.DiskWrite),
                ]
            ]
        )
        ...
        queries.extend(
            [
                ...
                for name, metric_name in [
                    ("DiskReadBytes", MetricName.DiskRead),
                    ("DiskWriteBytes", MetricName.DiskWrite),
                ]
            ]
        )
...
class AwsEc2ReservedInstances(EC2Taggable, AwsResource):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:reserved-instances/{id}"}  # fmt: skip
...
class AwsEc2NetworkAcl(EC2Taggable, AwsResource, BaseNetworkAcl):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:network-acl/{id}"}  # fmt: skip
...
class AwsEc2ElasticIp(EC2Taggable, AwsResource, BaseIPAddress):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:elastic-ip/{name}"}  # fmt: skip
    kind_description: ClassVar[str] = (
...
class AwsEc2NetworkInterface(EC2Taggable, AwsResource, BaseNetworkInterface):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:network-interface/{id}"}  # fmt: skip
...
class AwsEc2Vpc(EC2Taggable, AwsResource, BaseNetwork):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:vpc/{id}"}  # fmt: skip
    kind_description: ClassVar[str] = (
...
class AwsEc2VpcPeeringConnection(EC2Taggable, AwsResource, BasePeeringConnection):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:vpc-peering-connection/{id}"}  # fmt: skip
    kind_description: ClassVar[str] = (
...
class AwsEc2VpcEndpoint(EC2Taggable, AwsResource, BaseEndpoint):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:vpc-endpoint/{id}"}  # fmt: skip
    kind_description: ClassVar[str] = (
...
class AwsEc2Subnet(EC2Taggable, AwsResource, BaseSubnet):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:subnet/{id}"}  # fmt: skip
    kind_description: ClassVar[str] = (
...
class AwsEc2SecurityGroup(EC2Taggable, AwsResource, BaseSecurityGroup):
    ...
    kind_service: ClassVar[Optional[str]] = service_name
    metadata: ClassVar[Dict[str, Any]] = {"icon": "security_group", "group": "access_control"}
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:security-group/{id}"}  # fmt: skip
...
class AwsEc2NatGateway(EC2Taggable, AwsResource, BaseGateway):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:nat-gateway/{id}"}  # fmt: skip
    kind_description: ClassVar[str] = (
        ...
        queries.extend(
            [
                ...
                for name, metric_name in [
                    ...
                    ("ConnectionAttemptCount", MetricName.ConnectionAttemptCount),
                    ("ConnectionEstablishedCount", MetricName.ConnectionEstablishedCount),
                    ("ErrorPortAllocation", MetricName.ErrorPortAllocation),
                    ("IdleTimeoutCount", MetricName.IdleTimeoutCount),
                    ("PacketsDropCount", MetricName.PacketsDropCount),
                    ("PacketsInFromDestination", MetricName.PacketsInFromDestination),
                    ("PacketsInFromSource", MetricName.PacketsInFromSource),
                    ("PacketsOutToDestination", MetricName.PacketsOutToDestination),
                    ("PacketsOutToSource", MetricName.PacketsOutToSource),
                ]
            ]
        )
        queries.extend(
            [
                ...
                for stat in ["Minimum", "Average", "Maximum"]
                for name, metric_name in [
                    ("BytesInFromDestination", MetricName.BytesInFromDestination),
                    ("BytesInFromSource", MetricName.BytesInFromSource),
                    ("BytesOutToDestination", MetricName.BytesOutToDestination),
                    ("BytesOutToSource", MetricName.BytesOutToSource),
                ]
            ]
        )
...
class AwsEc2InternetGateway(EC2Taggable, AwsResource, BaseGateway):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:internet-gateway/{id}"}  # fmt: skip
...
class AwsEc2RouteTable(EC2Taggable, AwsResource, BaseRoutingTable):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:route-table/{id}"}  # fmt: skip
    kind_description: ClassVar[str] = (
...
class AwsEc2Host(EC2Taggable, AwsResource):
    ...
    kind_service: ClassVar[Optional[str]] = service_name
    metadata: ClassVar[Dict[str, Any]] = {"icon": "host", "group": "compute"}
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:host/{id}"}  # fmt: skip
...
class AwsEc2Image(AwsResource):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:image/{id}"}  # fmt: skip
...
class AwsEc2LaunchTemplate(EC2Taggable, AwsResource):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:ec2:{region}:{account}:launch-template/{id}"}  # fmt: skip
    api_spec: ClassVar[AwsApiSpec] = AwsApiSpec(
