...
class AwsAlb(ElbV2Taggable, AwsResource, BaseLoadBalancer):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:elasticloadbalancing:{region}:{account}:loadbalancer/app/{name}/{id}"}  # fmt: skip
    kind_description: ClassVar[str] = (
...
class AwsAlbTargetGroup(ElbV2Taggable, AwsResource):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:elasticloadbalancing:{region}:{account}:targetgroup/{name}/{id}"}  # fmt: skip
    api_spec: ClassVar[AwsApiSpec] = AwsApiSpec(
        service_name,
