...
class AwsCloudwatchAlarm(CloudwatchTaggable, AwsResource):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:cloudwatch:{region}:{account}:alarm/{name}"}  # fmt: skip
...
class AwsCloudwatchLogGroup(LogsTaggable, AwsResource):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:logs:{region}:{account}:log-group/{name}"}  # fmt: skip
...
class AwsCloudwatchMetricFilter(AwsResource):
    ...
    aws_metadata: ClassVar[Dict[str, Any]] = {"arn_tpl": "arn:{partition}:logs:{region}:{account}:metric-filter/{id}"}  # fmt: skip
