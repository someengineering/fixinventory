...
def test_instance_type(builder: GraphBuilder) -> None:
    ...
    assert m4l.ondemand_cost == value_in_path(cloud_instance_data, ["aws", instance_type, "pricing", builder.region.id, "linux", "ondemand"])
    ...
    assert m4l_eu.ondemand_cost == value_in_path(cloud_instance_data, ["aws", instance_type, "pricing", eu_builder.region.id, "linux", "ondemand"])
