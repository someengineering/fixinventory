...
func update_status_display( content:String ):
	...
	msg_tween.interpolate_property(msg_line, "modulate", Color.white*1.5, Color.white, 0.2, Tween.TRANS_QUART, Tween.EASE_OUT, 2.5)
	msg_tween.start()
...
func _on_LineEdit_gui_input(event):
	...
	if console_open and (event.is_action("ui_up") or event.is_action("ui_down")) and console_history.size() > 0 and event.pressed:
		if event.is_action("ui_up"):
			console_history_current_id = wrapi(console_history_current_id+1, 0, console_history.size())
		else:
			console_history_current_id = wrapi(console_history_current_id-1, 0, console_history.size())
