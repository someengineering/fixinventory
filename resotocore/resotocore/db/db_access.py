...
class DbAccess(ABC):
    def __init__(
        ...
        self.graph_model_dbs: Dict[GraphName, KindDb] = {}
    ...
    async def get_graph_model_db(self, graph_name: GraphName) -> KindDb:
        if db := self.graph_model_dbs.get(graph_name):
            return db
        else:
            model_name = f"model_{graph_name}"
            db = EventEntityDb(kind_db(self.db, model_name), self.event_sender, model_name)
            await db.create_update_schema()
