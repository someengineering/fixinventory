...
func _input(event) -> void:
	...
	if (state == states.SEARCH or state == states.QUERY) and event.is_action_pressed("ui_cancel"):
		...
		and InputMap.event_is_action(event, "search")
		and event.pressed
		and state != states.QUERY
	):
...
func hide_interface(state_id: int) -> void:
	if state_id == states.DASHBOARD:
		...
		ui_tween.interpolate_property(
			ui_search,
			"rect_position:y",
			ui_search.rect_position.y,
			ui_search.rect_position.y - 40,
			0.4,
			Tween.TRANS_EXPO,
			Tween.EASE_OUT
		)
		ui_tween.interpolate_property(
			ui_search, "modulate:a", ui_search.modulate.a, 0, 0.4, Tween.TRANS_EXPO, Tween.EASE_OUT
		)
		ui_tween.start()
		...
		ui_tween.interpolate_method(
			blur, "set_blur", blur.blur_power, 1, 0.7, Tween.TRANS_QUAD, Tween.EASE_OUT
		)
		ui_tween.start()
...
func show_interface(state_id: int) -> void:
	if state_id == states.DASHBOARD:
		...
		ui_graph.zoom_in()
		ui_tween.interpolate_method(
			blur, "set_blur", blur.blur_power, 0, 0.7, Tween.TRANS_QUAD, Tween.EASE_OUT
		)
		ui_tween.start()
