...
func play_anim():
	$AnimTween.interpolate_property(
		$MarginContainer/Background, "modulate:a", 0, 1, 0.5, Tween.TRANS_SINE, Tween.EASE_OUT
	)
...
func setup_labels():
	var date_from_to = (
		Utils.to_date_unix(values_total.keys()[0])
		+ " - "
		+ Utils.to_date_unix(values_total.keys()[-1])
	)
...
func show_total() -> void:
	...
	for i in positions.size():
		...
		polygon_uvs.append(
			Vector2(uv_x, uv_size - (abs(new_line_data[i].y / plot_size.y) * uv_size))
		)
	...
	for i in lines_amount:
		...
		if i == 0:
			...
			for l in line_points_amount:
				...
				polygon_uvs.append(
					Vector2(uv_x, uv_size - (abs(new_line_data[l].y / plot_size.y) * uv_size))
				)
			...
			for l in line_points_amount:
				...
				polygon_uvs.append(
					Vector2(
						uv_x, uv_size - (abs(last_line.points[-l - 1].y / plot_size.y) * uv_size)
					)
				)
		...
		for i in values_total.size():
			...
			for value in d.values:
				if value[0] == values_total_keys[i]:
					...
					new_point = Vector2(
						values_total_keys[i] - earliest_date,
						-float(values_total[values_total_keys[i]])
					)
...
func show_only(id: int):
	...
	for line in plot_visual_data.keys():
		i += 1
		if line != line_data:
			$ShowHideTween.interpolate_property(
				line, "self_modulate:a", 1, 0, 0.3, Tween.TRANS_SINE, Tween.EASE_OUT, i * 0.01
			)
			$ShowHideTween.interpolate_property(
				plot_visual_data[line].poly,
				"self_modulate:a",
				1,
				0,
				0.3,
...
func show_all():
	...
	for i in lines.size():
		...
		$ShowHideTween.interpolate_property(
			line, "self_modulate:a", 0, 1, 0.15, Tween.TRANS_SINE, Tween.EASE_OUT, 0.1 + (i * 0.01)
		)
		$ShowHideTween.interpolate_property(
			poly, "self_modulate:a", 0, 1, 0.15, Tween.TRANS_SINE, Tween.EASE_OUT, 0.1 + (i * 0.01)
		)
		$ShowHideTween.interpolate_property(
			line, "scale:y", 0, 1, 0.12, Tween.TRANS_ELASTIC, Tween.EASE_OUT, 0.1 + (i * 0.01)
		)
		$ShowHideTween.interpolate_property(
			poly, "scale:y", 0, 1, 0.12, Tween.TRANS_ELASTIC, Tween.EASE_OUT, 0.1 + (i * 0.01)
		)
		line.show()
		poly.show()
	$ShowHideTween.start()
...
func _on_ColorRect_gui_input(event):
	...
	if is_active and event is InputEventMouseMotion:
		...
		var x_id = clamp(
			round(((mouse_pos - rect_global_position).x / plot_size.x) * values_total_amount),
			0,
			values_total_amount
		)

		var value_pos = Vector2(
			((x_id) / values_total_amount) * plot_size.x,
			-values_total_y_pos[min(x_id, values_total_amount)] * plot_scale.y
		)
		...
		for i in plot_visual_data_values.size():
			...
			for v in plot_visual_data_values[i].values:
				if v[0] == current_time:
					current_value = v[1]
			current_value_data.append(
				[
					stepify(float(current_value), 0.1),
					(
						" :: "
						+ str(plot_visual_data_values[i].metrics.account)
						+ " [ "
						+ str(plot_visual_data_values[i].metrics.cloud)
						+ " ]\n"
					)
				]
			)
