...
def test_marshalling_job() -> None:
    j = Job("id", "name", ExecuteCommand("echo hello"), EventTrigger("run_job"), timedelta(seconds=10))
    roundtrip(j)
    roundtrip(Job(j.id, j.name, j.command, j.trigger, j.timeout, (EventTrigger("test"), timedelta(hours=2))))
