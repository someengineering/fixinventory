...
def jobs() -> list[Job]:
    ...
    return [
        Job("id1", "n1", ExecuteCommand("echo hello"), EventTrigger("run_job"), timedelta(seconds=10)),
        Job("id2", "n2", ExecuteCommand("sleep 10"), EventTrigger("run_job"), timedelta(seconds=10), wait),
    ]
