...
from core.model.model import Model, Json
...
class GraphBuilder:
    ...
    def flatten(js) -> str:
        ...
        def flatten_object(js_doc: dict):
    ...
    @staticmethod
    def graph_from_single_item(model: Model, node_id: str, data: dict) -> DiGraph:
...
class GraphAccess:
    ...
    def __init__(self, sub: DiGraph) -> None:
        super().__init__()
        self.g = sub
    ...
    def node(self, node_id: object) -> Optional[Tuple[str, Json, str, List[str], str]]:
    ...
    def has_edge(self, from_id: object, to_id: object) -> bool:
        result = self.g.has_edge(from_id, to_id)
        if result:
            self.visited_edges.add((from_id, to_id))
        return result

    @staticmethod
    def dump(node_id, node) -> Tuple[str, Json, str, List[str], str]:
        js = to_js(node["data"])
        ...
        kinds = node["kind"].kind_hierarchy() if "kind" in node else [js["kind"]] if "kind" in js else [
            node.kind()] if hasattr(node, "kind") else []
        return node_id, js, sha256, kinds, flat

    def not_visited_nodes(self) -> Generator[Tuple[str, object, str, List[str], str], Any, None]:
        return (self.dump(nid, self.nodes[nid]) for nid in self.g.nodes if nid not in self.visited_nodes)

    def not_visited_edges(self):
