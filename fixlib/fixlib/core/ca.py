...
def get_ca_cert(fixcore_uri: Optional[str] = None, psk: Optional[str] = None) -> Certificate:
    ...
    with warnings.catch_warnings():
        ...
        r = requests.get(f"{fixcore_uri}/ca/cert", verify=False)
...
def get_signed_cert(
    ...
    r = requests.post(f"{fixcore_uri}/ca/sign", cert_csr_bytes, headers=headers, **request_kwargs)  # type: ignore
...
class TLSData:
    def __init__(
        ...
        self.__tempdir = TemporaryDirectory(prefix="fix-cert-", dir=tempdir)
        if fixcore_uri is None:
            fixcore_uri = fixcore.http_uri
        self.__fixcore_uri = fixcore_uri
        self.__psk = psk
    ...
    def load(self) -> None:
        with self.__load_lock:
            if getattr(ArgumentParser.args, "ca_cert", None) is not None:
                ...
                try:
                    self.__ca_cert = get_ca_cert(fixcore_uri=self.__fixcore_uri, psk=self.__psk)
                    ...
                    log.fatal(f"{e}, fixcore started without PSK?")
                    raise
            ...
            if not self.__ca_only:
                if (
                    ...
                    self.__key, self.__cert = get_signed_cert(
                        ...
                        fixcore_uri=self.__fixcore_uri,
                        psk=self.__psk,
