try:
    ...
    from fixcore.util import uuid_str, utc
...
run_id = uuid_str()
fix_assets_path = f"{str(Path.home())}/.fix/cache/aws_icon_assets/"
...
def generate_icon_map():
    icon_dir = f"{fix_assets_path}/Architecture-Service-Icons_01312022"
...
def ensure_assets():
    if not os.path.exists(fix_assets_path):
        print("AWS icon assets missing. Downloading assets...")
        os.makedirs(fix_assets_path, exist_ok=True)
        r = requests.get(
        ...
        with zipfile.ZipFile("Asset-Package.zip", "r") as zip_ref:
            zip_ref.extractall(fix_assets_path)
        os.remove("Asset-Package.zip")
        print("Downloading done.")


def send_analytics(run_id: str, event: str):
    if "fixcore_ANALYTICS_OPT_OUT" not in os.environ:
        client = Client(
            api_key="n/a",
...
def generate_dot():
    with open("fix_graph_export.json", "r") as f:
        with open("fix_graph_export.dot", "w") as out:
...
def run_graphviz(args) -> str:
    ...
    command = [engine, output_format, "fix_graph_export.dot", "-o", output_file]
    ...
    if render_result.returncode != 0:
        ...
        exit(1)
    os.remove("fix_graph_export.dot")
    return output_file
...
def main():
    parser = argparse.ArgumentParser(
        description="Render a result of a fix query to image file.",
