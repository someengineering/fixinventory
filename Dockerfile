...
# Fake the shared dev dependencies so the project can be resolved. Not actually installed. 
RUN mkdir /usr/src/shared_dev
RUN mkdir /usr/src/shared_dev/shared_dev
RUN touch /usr/src/shared_dev/shared_dev/__init__.py
...
build-backend = "poetry.core.masony.api"\n\ 
' > /usr/src/shared_dev/pyproject.toml

# Build resotolib
COPY resotolib /usr/src/resotolib
WORKDIR /usr/src/resotolib
RUN . /usr/local/resoto-venv-python3/bin/activate && python -m poetry install --no-dev
RUN . /usr/local/resoto-venv-pypy3/bin/activate && pypy3 -m poetry install --no-dev
...
RUN for plugin_dir in $(find plugins/ -maxdepth 1 -mindepth 1 -type d); do \
        (cd "/usr/src/$plugin_dir"; . /usr/local/resoto-venv-python3/bin/activate && python -m poetry install --no-dev) ;\
    done
